...
### 4.3 過去時点の保有商品・評価額の再現範囲

2026年7月31日時点の実アカウント確認では、Money Forwardの資産履歴は現在取得できるスナップショットだけが表示され、任意の日付のページを開けても過去の資産明細が存在しない日付は空になる。`/bs/portfolio`も現在の保有商品を表示する画面であり、過去日を指定して当時の保有商品・保有数量・評価額を参照する機能は確認できていない。

楽天証券側についても、ユーザー確認により過去時点の保有商品・数量を月初基準で参照できる履歴が見つからなかった。

したがって、取引履歴だけから「各月1日時点で何を何株持っていたか」と「その日の評価額」を正確に復元することは、現在の取得データの範囲ではできない。全期間取得を実行しても、Money Forwardまたは接続先に保存されている履歴以上の過去ポートフォリオは生成されない。

月次の資産推移を追加する場合は、次のどちらかを明示的に選ぶ必要がある。

- 取得できた資産スナップショットだけを表示し、欠落月は欠落として扱う
- 売買履歴から保有数量を推定し、外部の過去株価・基準価額で評価する（推定値として表示する）

推定値を実績値として表示してはならない。非取引日の月初、投資信託の基準価額の公表タイミング、売買の受渡日も別途定義する必要がある。
...
通常更新でDBに存在しない口座またはカードを検出した場合、そのプロファイルは自動的に全期間取得へ切り替える。全期間取得はMoney Forward側で閲覧できる最古月まで進み、当月は通常取得した最新データを正として履歴データでは置換しない。

無料会員からプレミアム会員へ変更して過去の閲覧可能期間が広がった場合は、対象プロファイルの「全期間を再取得」を一度実行する。探索は画面上で前月へ進めなくなった時点で終了する。無限ループ防止の安全上限は既定360か月とし、必要な場合だけ`HISTORY_MAX_MONTHS`（1〜600）で変更する。
